print("[PUDGEWARS] EXECUTING ABILITYHOOK.LUA")
...
local function CreateHookChainForPlayer( nPlayerID , hero , Origin , headPosition )
    local nFXIndex = ParticleManager:CreateParticle("TODO pudge_meat_hook_chain",PATTACH_CUSTOM_ORIGIN,hero,hero)
...
function OnHookStart(keys)
    ...
    local uHead = nil
    while not uHead do
        -- CreateHookHeadForPlayer(nPlayerID , hero , heroPosition , headCreatedPosition)
        uHead = CreateHookHeadForPlayer(nPlayerID , caster , caster:GetOrigin() , caster:GetOrigin() )
    end
    tHookElements[nPlayerID].Head.unit = uHead

    -- create hook chain particle effect
    local nFXIndex = nil
    while not nFXIndex do
        -- CreateHookChainForPlayer( nPlayerID , hero , Origin , headPosition )
        nFXIndex = CreateHookChainForPlayer(nPlayerID , caster , caster:GetOrigin() , uHead:GetOrigin() )
    end
...
function OnHookSet(keys)
    ...
    while not uHead do
        -- CreateHookHeadForPlayer(nPlayerID , hero , heroPosition , headCreatedPosition)
        uHead = CreateHookHeadForPlayer(nPlayerID , caster , targetPoint , targetPoint )
    end
    tHookElements[nPlayerID].Head.unit = uHead
...
local function SetBackingHookHeadPosition(caster , uHead , nPlayerID)
    ...
    local diffVec = uHead:GetOrigin() - caster:GetOrigin()
    local vec3 = Vector(
        headOrigin.x - diffVec.x/20 + headFV.x * PER_HOOK_BODY_LENGTH * tnPlayerHookSpeed[nPlayerID]
       ,headOrigin.y - diffVec.y/20 + headFV.y * PER_HOOK_BODY_LENGTH * tnPlayerHookSpeed[nPlayerID]
       ,headOrigin.z
...
function OnReleaseHook( keys )
    ...
    if not tbPlayerFinishedHook[nPlayerID] then
        ...
        -- if reachs end
        if distance(uHead:GetOrigin(),caster:GetOrigin()) < 70 then
